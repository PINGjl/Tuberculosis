...
tt=lapply(data.list[c(1:18,19:36)], HTseq.handling,46)
head(tt[[1]])
tail(tt[[1]])
head(tt[[6]])
...
data.all=data.all[,c(1,20:37,2:19)]##换列 对照在前 实验在后
...
all.sample=c(paste(all.name[1],c(1:18),sep='-'),paste(all.name[2],c(1:18),sep='-'))
samplecondition=factor(c(rep(all.name[1],18),rep(all.name[2],18)),
